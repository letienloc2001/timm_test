...
class LoadBase64Images:
    def __init__(self, data, img_size=640, stride=32, auto=True):
        assert isinstance(data, list), "data must be a list"
        
        images = []
        for record in data:
            if isinstance(record, str):
                im_bytes = base64.b64decode(record)
                im_arr = np.frombuffer(im_bytes, dtype=np.uint8)
                raw_image = cv2.imdecode(im_arr, flags=cv2.IMREAD_COLOR)
                images.append(raw_image)
            else:
                raise Exception(f'ERROR: image is not match type base64 or np.array')
        ni = len(images)
    ...
    def __next__(self):
        ...
        assert img0 is not None, f'Image Not Found. ID: {self.count}'
        ...
        img = img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img = np.ascontiguousarray(img)
        return self.count, img, img0, self.cap, s
